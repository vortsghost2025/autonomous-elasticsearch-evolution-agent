...
class AgentOrchestrator {
  ...
  async initialize() {
    ...
    this.websocketServer = new WebSocketServer({ port: this.config.orchestration.masterAgent.port + 100 });
  ...
  launchAgent(profileId, customConfig = {}) {
    ...
    const childProcess = spawn('node', [`start-${profile.mode}-agent.js`, ...agentArgs], {
      stdio: ['pipe', 'pipe', 'pipe', 'ipc'],
      env: { ...process.env, AGENT_PROFILE: profileId, AGENT_PORT: profile.port }
    });
