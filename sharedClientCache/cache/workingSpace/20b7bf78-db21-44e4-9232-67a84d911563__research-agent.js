...
let broadcastInsight = () => {};
let broadcastStatus = () => {};

try {
  const dashboard = await import('./dashboard-server.js');
  if (dashboard.broadcastInsight && dashboard.broadcastStatus) {
    broadcastInsight = dashboard.broadcastInsight;
    broadcastStatus = dashboard.broadcastStatus;
    dashboardAvailable = true;
    console.log('[ResearchAgent] Connected to real-time dashboard server');
  }
} catch (e) {
  console.log('[ResearchAgent] Dashboard server not available, continuing without real-time updates');
}
...
export class ResearchAgent {
  ...
  async initialize() {
    ...
    try {
      ...
      if (dashboardAvailable) {
        broadcastStatus({
          researchStatus: `Active | ${this.insights.length} insights, ${this.patterns.length} patterns`
        });
      }
    } catch (error) {
  ...
  async analyzeClusterMetrics(metrics) {
    ...
    // Use AI agent to perform advanced analysis on specific issues
    try {
      const aiInsights = await this.getAIInsights(metrics);
      insights.push(...aiInsights);
    } catch (error) {
      console.warn('[ResearchAgent] AI analysis failed, continuing with standard analysis:', error.message);
    }
  ...
  /**
   * Get advanced insights from AI agent
   */
  async getAIInsights(metrics) {
    const aiInsights = [];
    
    // Dynamically import the AI agent to avoid initialization issues when not needed
    let agent = null;
    try {
      const agentModule = await import('./lib/agent.js');
      agent = agentModule.default;
    } catch (error) {
      console.warn('[ResearchAgent] Could not load AI agent, skipping AI analysis:', error.message);
      return aiInsights; // Return empty array if AI agent not available
    }
    
    // Check if we have API keys for LLM
    if (!process.env.ANTHROPIC_API_KEY && !process.env.OPENAI_API_KEY) {
      console.log('[ResearchAgent] No LLM API keys found, skipping AI analysis');
      return aiInsights;
    }
    
    // Perform AI analysis on various aspects of the cluster
    const analysisPromises = [];
    
    // Analyze performance patterns
    analysisPromises.push(this.performAIAnalysis(
      'Analyze performance patterns in these cluster metrics and identify optimization opportunities',
      metrics,
      agent
    ));
    ...
    analysisPromises.push(this.performAIAnalysis(
      'Analyze resource utilization patterns and recommend optimization strategies',
      metrics,
      agent
    ));
    ...
    analysisPromises.push(this.performAIAnalysis(
      'Analyze index efficiency based on fragmentation and storage metrics',
      metrics,
      agent
    ));
    ...
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value) {
        ...
        console.warn('[ResearchAgent] AI analysis failed:', result.reason?.message);
      }
    }
  ...
  /**
   * Perform AI analysis on specific metrics
   */
  async performAIAnalysis(question, metrics, agent) {
    try {
      const result = await agent.ask(question, {
        summarize: true,
        provider: 'claude', // Using Claude for analytical tasks
        returnRaw: false
      });
      ...
      if (result.success && result.summary) {
        return {
          type: 'ai-enhanced',
          category: 'analysis',
          description: result.summary.substring(0, 200) + (result.summary.length > 200 ? '...' : ''), // Truncate if too long
          severity: 'info', // AI insights start as informational
          recommendation: 'Implement AI-recommended optimization strategy'
        };
      }
    } catch (error) {
      console.warn('[ResearchAgent] Failed to get AI analysis:', error.message);
    }
  ...
  async generateResearchReport(metrics, metricsHistory = []) {
    ...
    // Broadcast status update if dashboard available
    if (dashboardAvailable) {
      broadcastStatus({
        researchStatus: `Active | ${this.insights.length} insights, ${this.patterns.length} patterns`
      });
  ...
  async getResearchSummary() {
    ...
    if (dashboardAvailable) {
      broadcastStatus({
        researchStatus: `Active | ${summary.insightsCount} insights, ${summary.patternsCount} patterns`
      });
    }
