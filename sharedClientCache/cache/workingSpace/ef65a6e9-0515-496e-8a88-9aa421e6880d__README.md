...
```bash
# Start the dashboard server (mock version)
npm start
...
2. **Set up port forwarding in Azure Portal**:
   ...
   - Local Port: `8000`
   - Remote Port: `3001` (the mock server port)
...
```bash
# Start the mock server (recommended for Azure Dev Tunnel)
node start-mock-server.js

# Or run the demo without dashboard
npm run demo
